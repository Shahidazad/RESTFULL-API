...
class Item(Resource):
    ...
    @fresh_jwt_required # reqiured fresh access token 
    def post(self, name):
        ...
        item = ItemModel(name, **data) # change
...
class ItemList(Resource):
    @jwt_optional
    def get(self):
        user_id = get_jwt_identity() # give us what we save as access token as identity
        items = [item.json() for item in ItemModel.find_all()] # change list comprehension
        if user_id:
